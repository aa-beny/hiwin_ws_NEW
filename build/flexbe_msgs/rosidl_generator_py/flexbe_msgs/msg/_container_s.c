...
bool flexbe_msgs__msg__container__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("flexbe_msgs.msg._container.Container", full_classname_dest, 36) == 0);
  }
  flexbe_msgs__msg__Container * ros_message = _ros_message;
  {  // path
  ...
  {  // transitions
    ...
    }
    {
      ...
      rosidl_runtime_c__String * dest = ros_message->transitions.data;
      for (Py_ssize_t i = 0; i < size; ++i) {
        PyObject * item = PySequence_Fast_GET_ITEM(seq_field, i);
        if (!item) {
          Py_DECREF(seq_field);
  ...
  {  // autonomy
    PyObject * field = PyObject_GetAttrString(_pymsg, "autonomy");
    if (!field) {
      return false;
    }
    if (PyObject_CheckBuffer(field)) {
      ...
      int8_t * dest = ros_message->autonomy.data;
      ...
      PyObject * seq_field = PySequence_Fast(field, "expected a sequence in 'autonomy'");
      if (!seq_field) {
        Py_DECREF(field);
        return false;
      }
...
{
  /* NOTE(esteve): Call constructor of Container */
  PyObject * _pymessage = NULL;
  {
    PyObject * pymessage_module = PyImport_ImportModule("flexbe_msgs.msg._container");
    assert(pymessage_module);
    PyObject * pymessage_class = PyObject_GetAttrString(pymessage_module, "Container");
  ...
  {  // transitions
    ...
    rosidl_runtime_c__String * src = ros_message->transitions.data;
    field = PyList_New(size);
    if (!field) {
      return NULL;
    }
  ...
  {  // autonomy
    PyObject * field = NULL;
    field = PyObject_GetAttrString(_pymessage, "autonomy");
    if (!field) {
      return NULL;
    }
    ...
    if (ros_message->autonomy.size > 0) {
      // populating the array.array using the frombytes method
      PyObject * frombytes = PyObject_GetAttrString(field, "frombytes");
      assert(frombytes != NULL);
      int8_t * src = &(ros_message->autonomy.data[0]);
      PyObject * data = PyBytes_FromStringAndSize((const char *)src, ros_message->autonomy.size * sizeof(int8_t));
      assert(data != NULL);
